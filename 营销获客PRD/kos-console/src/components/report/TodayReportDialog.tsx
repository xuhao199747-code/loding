...
export function TodayReportDialog({ open, onClose, onGoSchedule }: { open: boolean; onClose: () => void; onGoSchedule?: () => void }) {
  return <Dialog open={open} onOpenChange={(value) => !value && onClose()}>
    <DialogContent aria-describedby={undefined}>
      ...
      <div className="flex justify-end gap-2"><Button variant="outline" onClick={onClose}><X className="h-4 w-4" />关闭</Button><Button onClick={() => { onClose(); onGoSchedule?.(); }}>查看跟进队列<ArrowUpRight className="h-4 w-4" /></Button></div>
    </DialogContent>
  </Dialog>;
}
